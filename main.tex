...
Our code \href{https://github.com/AstroJacobLi/kuaizi}{\code{kuaizi}} is also available on GitHub. 
...
In Figure \ref{fig:qfrac}, the green lines show the quenched fractions $f_q$ as a function of stellar mass for the whole UDG (\textit{left}) and UPG (\textit{right}) samples, and the error bars correspond to the $1\sigma$ Bernoulli standard error. The quenched fraction of UDGs is high ($\sim 0.7$) over $6.8 < \log\ M_\star/M_\odot < 8.5$ and weakly depends on the stellar mass. On the contrary, as increasing stellar mass, the quenched fraction of UPGs decreases from $f_q \approx 0.8$ at $10^{6.8}\ M_\odot$ to $f_q \approx 0.1$ at $10^{8.5}\ M_\odot$. 

We notice that the UDG sample reaches to $\log\,M_\star\approx 8.3$ whereas UPG sample reaches to $\log\,M_\star\approx 9.0$. This is because $10^9\ M_\odot$ galaxies no longer satisfy the surface brightness cut for UDG. We are only able to probe galaxies down to $10^6.8\ M_\odot$ due to our detection limit. 



Say we don't find dependence on host stellar mass (no fig here)

They derived the quenched fraction based on morphology, and they also find that 

It is shown that the morphology correlates with the 
1
the SFR is correlated with other 

calculate the fraction of quenched 
...
\subsection{High quenched fraction of UDGs is an artifact}
