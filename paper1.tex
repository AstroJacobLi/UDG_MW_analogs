...
However, in order to construct a UPG sample, one needs to know the distance, the color, a color-$M_\star/L$ relation to convert observed magnitudes to stellar mass, and a mass-size relation and the scatter therein. None of these are easy to obtain or free from systematic errors. Both size and stellar mass could have large uncertainties in the LSB regime. One also needs to apply a surface brightness cut (such as $\sbeff > 23.0\ \sbunit$ in this work) to make sure that the UPG sample is not dominated by disk galaxies with high surface brightness due to the bulges. In \S\ref{sec:bkg}, we find that the UPG sample has a higher contamination fraction than the UDG sample. Since the UPG definition does not preferentially exclude blue LSBGs, background disk galaxies are more easily to be identified as UPGs. Based on our test, a UPG sample with more aggressive cut (e.g., $>2\sigma$ above the average mass-size relation) would have a higher purity, but only possible when one have a much larger LSBG sample.
